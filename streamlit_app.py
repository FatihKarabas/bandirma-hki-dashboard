...
HKİ Emoji Fonksiyonu

def get_hki_icon(hki_value): if hki_value <= 50: return "🟢 😊 (İyi)" elif hki_value <= 100: return "🟡 😐 (Orta)" elif hki_value <= 150: return "🟠 😷 (Hassas Gruplar için)" elif hki_value <= 200: return "🔴 🤒 (Sağlıksız)" elif hki_value <= 300: return "🟣 😫 (Kötü)" else: return "⚫ ☠️ (Tehlikeli)"
...
with st.sidebar: st.image("https://upload.wikimedia.org/wikipedia/commons/thumb/2/2f/Logo_Band%C4%B1rma.png/600px-Logo_Band%C4%B1rma.png", width=120) st.title("🌍 Bandırma HKİ (Hava Kalitesi İndeksi)") st.markdown("Hava Kalitesi Bilgilendirme Platformu") st.markdown("---")
...
Seçilen tarih için satır

selected_row = df[df['datetime'].dt.date == selected_date] if not selected_row.empty: row = selected_row.iloc[0] st.markdown(f""" - HKİ: {row['hki']} {get_hki_icon(row['hki'])} - Kategori: {row['hki_kategori']} - Renk: {row['hki_renk']} - Açıklama: {row['hki_aciklama']} - Belirleyici Kirletici: {row['hki_kaynak']} """) else: st.warning("Seçilen tarihte veri bulunamadı.")
...
def generate_pdf(row, best_row, worst_row, plot_path): pdf = FPDF() pdf.add_page() pdf.add_font('DejaVu', '', 'DejaVuSans.ttf', uni=True) pdf.add_font('DejaVu', 'B', 'DejaVuSans-Bold.ttf', uni=True) pdf.set_font('DejaVu', 'B', 16) pdf.cell(0, 10, 'Bandırma Hava Kalitesi Raporu', ln=True, align='C') pdf.ln(10) pdf.set_font('DejaVu', '', 12) pdf.multi_cell(0, 10, f""" Tarih: {row['datetime'].date()} HKİ: {row['hki']} - {get_hki_icon(row['hki'])} Kategori: {row['hki_kategori']} Renk: {row['hki_renk']} Açıklama: {row['hki_aciklama']} Belirleyici Kirletici: {row['hki_kaynak']}

En İyi HKİ (Dönem): {best_row['hki']} - {best_row['datetime']} {get_hki_icon(best_row['hki'])}
En Kötü HKİ (Dönem): {worst_row['hki']} - {worst_row['datetime']} {get_hki_icon(worst_row['hki'])}
""")
pdf.image(plot_path, x=10, y=None, w=180)
return pdf.output(dest='S').encode('latin-1')

Matplotlib grafiği + geçici kayıt
...
st.header(f"📈 {period} İçin En İyi ve En Kötü HKİ Analizi") if not period_df.empty: st.subheader("✅ En İyi HKİ Verisi") st.markdown(f""" - Tarih/Saat: {best_row['datetime']} - HKİ: {best_row['hki']} {get_hki_icon(best_row['hki'])} - Kategori: {best_row['hki_kategori']} - Renk: {best_row['hki_renk']} - Kirletici: {best_row['hki_kaynak']} - Açıklama: {best_row['hki_aciklama']} """)
