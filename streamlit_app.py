...
if generate: if not selected_row.empty and not period_df.empty: best_row = period_df.loc[period_df["hki"].idxmin()] worst_row = period_df.loc[period_df["hki"].idxmax()]
...
fig2, ax = plt.subplots(figsize=(10, 4))
    ...
    st.download_button(
        ...
        file_name="hava_kalitesi_raporu.pdf",
        mime="application/pdf"
    )
else:
    st.warning("PDF oluşturmak için yeterli veri yok.")

st.header(f"📈 {period} İçin En İyi ve En Kötü HKİ Analizi") if not period_df.empty: best_row = period_df.loc[period_df["hki"].idxmin()] worst_row = period_df.loc[period_df["hki"].idxmax()]

st.subheader("✅ En İyi HKİ Verisi")
st.markdown(f"""
- **Tarih/Saat:** {best_row['datetime']}
- **HKİ:** {best_row['hki']} {get_hki_icon(best_row['hki'])}
- **Kategori:** {best_row['hki_kategori']}
- **Renk:** {best_row['hki_renk']}
- **Kirletici:** {best_row['hki_kaynak']}
...
else: st.warning(f"{period} için geçerli veri bulunamadı.")
